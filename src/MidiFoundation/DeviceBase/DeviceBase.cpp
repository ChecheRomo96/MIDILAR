...
namespace MIDILAR::MidiFoundation {
    ...
    void DeviceBase::MidiInput(const MidiFoundation::Message& message) {
        MidiInput(message.Buffer(), message.size());
    }

    void DeviceBase::MidiOutput(const MidiFoundation::Message& message) {
        MidiOutput(message.Buffer(), message.size());
    }

    void DeviceBase::Update(SystemFoundation::Clock::TimePoint SystemTime) {
        // Default implementation - derived classes can override
